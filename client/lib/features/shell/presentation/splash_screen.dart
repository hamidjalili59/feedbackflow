...
class SplashScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return GradientScaffold(
      appBar: AppBar(actions: const [LanguageButton(), ThemeModeButton(), SizedBox(width: 12)]),
      body: Center(
        child: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            Icon(Icons.fact_check_rounded, size: 64, color: Theme.of(context).colorScheme.primary),
            const SizedBox(height: 18),
            const CircularProgressIndicator(),
            const SizedBox(height: 14),
            Text(context.l10n.t('preparingApp'), style: Theme.of(context).textTheme.titleMedium),
          ],
        ),
      ),
    );
